...
def main(line, speciesPath):
    ...
    if os.path.exists(folder_path):
        ...
        if raw_file_count != 0:
            ...
            if os.path.exists(os.path.join(speciesPath_dir, '00.rawdata')):
                if os.listdir(os.path.join(speciesPath_dir, '00.rawdata')):
                    raw_file_count = len([file for file in os.listdir(os.path.join(speciesPath_dir, '00.rawdata')) if file.endswith('.raw')])
                else:
    ...
    # 3.Generate workdir
    if not os.path.exists(os.path.join(speciesPath_dir, '00.rawdata')):
    ...
    if len([f for f in os.listdir(os.path.join(speciesPath_dir, '00.rawdata')) if f.endswith(".raw")]) == 0:
    ...
    # 6.msConvert run
    if len([f for f in os.listdir(os.path.join(speciesPath_dir, '00.rawdata')) if f.endswith(".mzML")]) == 0:
        # First run
        raw_dir = "00.rawdata"
    ...
    raw_files = [file.removesuffix('_results.sage.tsv') for file in os.listdir(list_file_path) if file.endswith('_results.sage.tsv')]
    failed_Sage = [name + '.raw' for name in names_list if name not in raw_files]
    
    # failed Fragpipe files
    list_file_path = os.path.join(speciesPath_dir, "05.All_results", "00.FragPipe")
    raw_files = [file.removesuffix('_percolator_target_psms.tsv') for file in os.listdir(list_file_path) if file.endswith('_percolator_target_psms.tsv')]
    failed_Frag = [name + '.raw' for name in names_list if name not in raw_files]
    
    species = line
    Nproject = sum(1 for item in os.listdir(folder_path) if os.path.isdir(os.path.join(folder_path, item)))
    ...
    NSage = len(names_list) - len(failed_Sage)
    NFrag = len(names_list) - len(failed_Frag)
